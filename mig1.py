...
def migration_source(model_path, script_path, matlab_version):
    ...
    epp_file = model_path.replace('.slx', '.epp').replace('Wrapper_', '')
...
def migration_target(project_path, model_path, script_path, matlab_version):
    ...
    ep.get(f'profiles/{project_path}?discardCurrentProfile=true')
    ...
    # Save *.epp
    ep.put('profiles', { 'path': project_path }, message="Saving profile")
